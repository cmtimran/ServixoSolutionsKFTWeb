  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-slate-400">Manage your core website configuration, contact details, and integrations.</p>
      </div>
      ...
      <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-6 sm:p-8 backdrop-blur-sm">
        <form onSubmit={handleSave} className="max-w-2xl space-y-10">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-400 pb-2">Enter your Stripe API keys to enable checkout and payments for digital products.</p>
            
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Stripe Public Key</label>
                <input
                  type="text"
                  value={settings.stripePublicKey}
                  onChange={(e) => setSettings({ ...settings, stripePublicKey: e.target.value })}
                  placeholder="pk_live_..."
                  className="w-full bg-slate-900/50 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-mono text-sm"
                />
              </div>
          ...
          <div className="pt-4 flex items-center justify-between border-t border-slate-800">
            ...
            <button
              type="submit"
              disabled={saving}
              className="mt-4 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 px-6 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {saving ? (
